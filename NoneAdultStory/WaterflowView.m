...
- (void) setNoMoreToLoad:(BOOL)pNoMoreToLoad {
    noMoreToLoad = pNoMoreToLoad;
    NSLog(@"noMoreToLoad...%d", noMoreToLoad);
    if (noMoreToLoad) {
        [_loadFooterView setNoMoreToLoad];
    }
}
...
- (void)cellSelected:(NSNotification *)notification
{
    /*if ([self.flowdelegate respondsToSelector:@selector(flowViewDidSelectCell:)])
    {
        [self.flowdelegate flowViewDidSelectCell:((WaterFlowCell*)notification.object)];
    }*/
}
...
- (void)initialize
{
    ...
    for (int i = 0; i< numberOfColumns; i++)
    {
        ...
        //NSLog(@"cellHeightInOneColume: %@", cellHeightInOneColume);
...
- (void)pageScroll
{
    ...
    for (int i = 0 ; i< numberOfColumns; i++)
    {
        ...
        if ([self.visibleCells objectAtIndex:i] == nil || ((NSArray*)[self.visibleCells objectAtIndex:i]).count == 0) //everytime reloadData is called and no cells in visibleCellArray
        {
			...
			{
                //NSLog(@"i: %d, j: %d", i, j);
                float everyCellHeight = 0;
                @try {
                    everyCellHeight = [[[self.cellHeight objectAtIndex:i] objectAtIndex:j] floatValue];
                }
...
//
            cell.flowdelegate = _flowdelegate;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    */
    if ([_flowdelegate respondsToSelector:@selector(flowViewDidSelectCell:)]) {
        NSLog(@"flowViewDidSelectCell, self.indexPath: %d, %d", self.indexPath.row, self.indexPath.section);
        [_flowdelegate flowViewDidSelectCell:self.indexPath];
    }
}
